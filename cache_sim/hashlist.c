...
int add_str(list l, ULONG slot_num, char *str, ULONG length) {
	ULONG pos = first_hash(str, length) % slot_num;
	ULONG start = pos;
...
int add_counter(list l, ULONG slot_num, char *str, ULONG length, ULONG total_inc) {
...
inline int get_id(list l, UINT slot) {
	return l[slot].id;
}

inline void set_size(list l, UINT slot, UINT size) {
	l[slot].size = size;
}

inline UINT get_size(list l, UINT slot) {
	return l[slot].size;
}
